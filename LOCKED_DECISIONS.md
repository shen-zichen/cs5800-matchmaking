...
|---|---|---|---|---|
| **Stage 1 · Pooling** | 按 MMR sort，用 sliding window 扩窗，以 matching 作 feasibility oracle，抽出一个 MMR 紧凑且 lane-feasible 的 10 人 pool P | Quicksort + sliding window（feasibility 检查用 max-flow） | Ch 7（+ Ch 26 判可行） | polynomial |
...
- **执行顺序：sort → match → balance**（match-first）。先 match 保证每 lane 2 人，切队时每队天然 role-feasible。
---
...
- lane-first：lane 优先、balance 让步 → autofill=0，gap 可能偏大。
- balance-first：balance 优先、lane 让步 → gap 最小，但可能 autofill。
...
配套实验：
- **对比实验**（P=10，控制变量，同一组 10 人喂两种 ordering）→ 展示 tradeoff 结构。
- **scalability 实验**（P 从 10 增大）→ 证明 matching 的 polynomiality 在大池选人时是必需的。
---
...
- unweighted、one-sided、capacity-2 bipartite matching；primary preference 仅作 tie-break。
