...
Lane matching determines whether a given set of players can be assigned to the five standard roles ($\text{TOP}, \text{JUG}, \text{MID}, \text{ADC}, \text{SUP}$) strictly within their stated primary or secondary preferences. This is where **role feasibility** is evaluated: a match is valid only if all five lanes are fully staffed. We model this assignment problem as a **Maximum Bipartite Matching** problem and solve it via **Max-Flow** in pure Python, following CLRS Chapter 26 (Maximum Flow).

### Residual Network Construction
We construct a directed flow network $G = (V, E)$ using a residual edge representation (`MatchingGraph`). The vertex set is $V = \{s, t\} \cup P \cup L$, where $P$ is the set of players and $L = \{\text{TOP}, \text{JUG}, \text{MID}, \text{ADC}, \text{SUP}\}$ is the set of five lanes. Edges and capacities are defined as follows:
* **Source to Players**: For each player $p \in P$, we add edge $(s, p)$ with capacity $\text{cap} = 1$, ensuring each player occupies at most one lane.
* **Players to Preferred Lanes**: If lane $\text{lane} \in L$ is player $p$'s primary preference, we add edge $(p, \text{lane})$ with capacity $\text{cap} = 1$. If a secondary preference exists, an additional edge $(p, \text{lane})$ with capacity $\text{cap} = 1$ is added. Non-preferred lanes are structurally omitted.
* **Lanes to Sink**: For each lane $\text{lane} \in L$, we add edge $(\text{lane}, t)$ with capacity $\text{capacity}$ (corresponding to parameter `lane_capacity`).

### Capacity Scaling and Dual Usage
The lane capacity parameter `lane_capacity` ($\text{capacity}$) configures the matcher for two symmetric usage modes across the pipeline:
* **Team Assignment ($\text{capacity} = 1, |P| = 5$)**: Requires each lane to seat exactly 1 player. It serves two purposes: evaluating or assigning roles for a single 5-player team, and executing Stage 2 within each 5-player half in the Balance-first pipeline (§2.4).
* **Pool Screening ($\text{capacity} = 2, |P| = 10$)**: Requires each lane to seat exactly 2 players. It also serves two purposes: verifying 10-player candidate pools during Stage 1 pooling, and executing full-pool role matching in the Lane-first pipeline (§2.4).

In both configurations, the total sink in-capacity equals the player count ($5 \times \text{capacity} = |P|$).

### Edmonds-Karp Solver and Autofill Extraction
We compute the maximum flow using the **Edmonds-Karp algorithm** (`solve_lane_matching`):
1. **Augmenting Path Search (`bfs_find_path`)**: Breadth-First Search finds the shortest path (fewest edges) from $s$ to $t$ in the residual network, returning a parent mapping.
2. **Path Reconstruction and Flow Update (`reconstruct_path` & `update_flow_along_path`)**: `reconstruct_path` traces back from $t$ to $s$ to construct the node path. `update_flow_along_path` computes the path bottleneck capacity (identically $1$ for unweighted bipartite matching) and updates residual capacities along the path.
3. **Flow Extraction and Autofill (`handle_autofill`)**: Matches are extracted from edges $(p, \text{lane})$ carrying unit flow. Feasibility is satisfied **if and only if** $\text{max\_flow} = |P|$. If $\text{max\_flow} < |P|$, a shortfall exists, and we record $\text{autofill} = |P| - \text{max\_flow}$; unmatched players are subsequently allocated to remaining open lanes via `handle_autofill` (setting `is_autofilled = True`).

### Structural Insight: Hall's Marriage Theorem
While Max-Flow algorithmically computes the assignment, **Hall's Marriage Theorem** provides the exact theoretical condition for feasibility:
1. **Hall's Condition**: A bipartite graph admits a complete assignment if and only if for every subset of players $S \subseteq P$, their collectively acceptable lanes $N(S)$ satisfy total capacity $|N(S)| \times \text{capacity} \ge |S|$.
2. **Project Domain Mapping**: Left vertices correspond to players, right vertices to the 5 lanes (capacity $\text{capacity}$). Feasibility requires every player group to have sufficient total capacity across their acceptable lanes.
3. **Overcrowded Subsets (Hall Violators)**: When a preference collision occurs — for example, in a 5-player team ($\text{capacity} = 1$), if 1 player accepts $\{\text{TOP}, \text{JUG}\}$ while the remaining 4 players collectively accept only $\{\text{MID}, \text{ADC}, \text{SUP}\}$ (total capacity $3 \times 1 = 3$), every single lane has interested players, yet the subset of 4 players exceeds total capacity ($4 > 3$), forming an **overcrowded subset** (Hall violator).
4. **Algorithmic Separation**: Hall's condition explains *why* max-flow falls short, forcing positive autofill. We do not enumerate subsets to verify Hall's condition directly; instead, Edmonds-Karp Max-Flow serves as an efficient $O(VE^2)$ polynomial oracle that computes max-flow directly.

### Implementation and Emergent Priority
Because $|V| \le 17$ and $|E| \le 27$, Edmonds-Karp runs in sub-millisecond time per invocation. Furthermore, since primary and secondary edges carry equal unit capacity, the matcher treats all preferences as unweighted. Primary preferences are favored purely as an emergent artifact of BFS traversing primary edges first during path search.
...
_Remark_ (_lane-first balances by the same reduction_). Lane-first's choice among its 32 lane-constrained splits (§2.4) is itself PARTITION in optimization form: it picks a sign for each lane's within-pair MMR difference so as to minimize the residual gap. Its general form is therefore NP-hard by a reduction from PARTITION — a cleaner one than §2.3.2, since with no equal-cardinality constraint to satisfy it needs no dummy-player gadget. We enumerate all 32 for exactly the reason we enumerate the 126: the fixed five-lane instance makes the search constant, not because the problem is easy.
